function  [ni, roiName] = dtiRoiNiftiFromMat_temp(matRoi,refImg,roiName,saveFlag)
% 
...
xform = ref.qto_xyz;
ref_dim_size = size(ref.data);
ref_dim_input = [ref_dim_size(1) ref_dim_size(2) ref_dim_size(3)];
pix_dim_size = ref.pixdim;
pix_dim_input = [pix_dim_size(1) pix_dim_size(2) pix_dim_size(3)];
bb = [-(ref_dim_input.*pix_dim_input)/2; (ref_dim_input.*pix_dim_input)/2-1];

%% Create the roiImg and xForm from the roi 
[roiImg, imgXform] = dtiRoiToImg_temp(roi,xform,bb);
