		...
		if [ `echo "${mergeROIsL}" | wc -l` -gt 1 ]; then
			...
			do
				...
				3dTcat -prefix merge_preL.nii.gz zeroDataset.nii.gz `ls ${mergeArrayL}`
		        	...
		        	rm -rf merge_preL.nii.gz ${mergeName}L_nonbyte.nii.gz ${mergeName}L_allbytes.nii.gz zeroDataset.nii.gz
	        	done
        	else
			mergeArrayL=""
			for i in "${mergeL[@]}"
		...
		if [ `echo "${mergeROIsR}" | wc -l` -gt 1 ]; then
			...
			do 
				...
				3dTcat -prefix merge_preR.nii.gz zeroDataset.nii.gz `ls ${mergeArrayR}`
		        	...
		        	rm -rf merge_preR.nii.gz ${mergeName}R_nonbyte.nii.gz ${mergeName}R_allbytes.nii.gz zeroDataset.nii.gz
			done
        	else
			mergeArrayR=""
			for i in "${mergeR[@]}"
